-- Jalankan file ini SEKALI di SQL Editor Supabase server Anda
-- untuk memasang mesin backup & restore (aman dijalankan berulang).
...
-- pengosongan tabel saat mode Replace (aman untuk tabel tanpa kolom id)
...
-- membersihkan kolom token akun login yang bernilai NULL setelah restore
-- (GoTrue menolak NULL dan menyebabkan gagal login)
